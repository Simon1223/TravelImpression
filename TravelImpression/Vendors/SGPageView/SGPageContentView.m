...
- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i<childVCs.count; i++) {
            if ([childVCs[i] isKindOfClass:[UIViewController class]]) {
                [childArray addObject:childVCs[i]];
            }
            else if ([childVCs[i] isKindOfClass:[NSString class]])
            {
                [childArray addObject:[[NSClassFromString(childVCs[i]) alloc] init]];
            }
        }
        self.childViewControllers = childArray;
