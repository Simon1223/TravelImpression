...
#import "SMPage3DFlowView.h"
#import "SMBannerSubiew.h"

@interface SMPage3DFlowView ()
...
@implementation SMPage3DFlowView
...
- (void)initialize{
    ...
    //UIPageControl控制器
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - 80 , self.bounds.size.width, 8)];
    ...
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width/2, 20)];
    self.titleLabel.center = CGPointMake(self.center.x, 80);
    self.titleCenter = self.titleLabel.center;
    self.titleLabel.backgroundColor = [UIColor colorWithWhite:0.3 alpha:0.8];
...
- (void)autoNextPage {
    ...
    switch (self.orientation) {
        case SMPage3DFlowViewOrientationHorizontal:{
            
            [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:YES];
            break;
        }
        case SMPage3DFlowViewOrientationVertical:{
    ...
    switch (self.orientation) {
        ...
        case SMPage3DFlowViewOrientationVertical:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.height || delta>_pageSize.height) {
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    SMBannerSubiew *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        if (!cell.superview) {
            cell.mainImageView.center = CGPointMake(_scrollView.frame.size.width/2, _scrollView.frame.size.height/2);
            cell.coverView.center = CGPointMake(_scrollView.frame.size.width/2, _scrollView.frame.size.height/2);
...
- (void)reloadData
{
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:@"SMBannerSubiew"]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case SMPage3DFlowViewOrientationHorizontal://横向
...
- (UIView *)dequeueReusableCell{
    SMBannerSubiew *cell = [_reusableCells lastObject];
    if (cell)
    {
